...
class QuantizedConv2dReLU(nn.Module):
    ...
    def forward(self, x):
        return conv2d.conv2d_relu_int8(x, self.weight, self.bias, self.stride, self.padding, self.dilation, True)
    
class QuantizedConv2d(nn.Module):
    """Quantized Conv2d without ReLU activation"""
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, bias=True):
        super().__init__()
        self.in_channels = in_channels
        self.out_channels = out_channels
    ...
    def forward(self, x):
        return conv2d.conv2d_relu_int8(x, self.weight, self.bias, self.stride, self.padding, self.dilation, False)
...
class TorchConv2d(nn.Module):
    """Baseline PyTorch Conv2d without ReLU activation"""
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, bias=True):
        super().__init__()
        self.conv = nn.Conv2d(in_channels, out_channels, kernel_size, stride=stride, padding=padding, dilation=dilation, bias=bias)
